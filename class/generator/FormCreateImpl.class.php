...
class FormCreateImpl extends FormCreate{
	...
	protected function generateIntegerInput($attr, $entity, $field) {
		$attr['type'] = "text";
		$attr['value'] = '<?php echo $' . strtolower($entity['name']) . '->get' . ucfirst($field['name']) . '(); ?>';
	...
	protected function generateDateInput($attr, $entity, $field) {
		...
		$attr['type'] = "text";
		$attr['value'] = '<?php echo $' . strtolower($entity['name']) . '->get' . ucfirst($field['name']) . '()->format("d/m/Y"); ?>';
		$this->xmlUtils->all("input", $attr);
	}
	...
	protected function generateDatetimeInput($attr, $entity, $field) {
		...
		$attr['type'] = "text";
		$attr['value'] = '<?php echo $' . strtolower($entity['name']) . '->get' . ucfirst($field['name']) . '()->format("d/m/Y H:m"); ?>';
		$this->xmlUtils->all("input", $attr);
	}	
}
